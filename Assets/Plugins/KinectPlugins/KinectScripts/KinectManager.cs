...
{
	[Tooltip("How high above the ground is the sensor, in meters. 传感器离地面有多高，以米为单位。")]
	public float sensorHeight = 1.0f;

	[Tooltip("Kinect elevation angle (in degrees). May be positive or negative. Kinect仰角（度）。可能是积极的或消极的。")]
	public float sensorAngle = 0f;
	
	public enum AutoHeightAngle : int { DontUse, ShowInfoOnly, AutoUpdate, AutoUpdateAndShowInfo }
	[Tooltip("Whether to automatically set the sensor height and angle or not. The user must stay in front of the sensor, in order the automatic detection to work. 是否自动设置传感器高度和角度。用户必须站在传感器前面，以便自动检测工作。")]
	public AutoHeightAngle autoHeightAngle = AutoHeightAngle.DontUse;

	[Tooltip("Whether to flip left and right, relative to the sensor. 是否相对于传感器左右翻转。")]
	private bool flipLeftRight = false;

	public enum UserMapType : int { None, RawUserDepth, BodyTexture, UserTexture, CutOutTexture }
	[Tooltip("Whether and how to utilize the user and depth images. 是否以及如何利用用户和深度图像。")]
	public UserMapType computeUserMap = UserMapType.RawUserDepth;
	
	[Tooltip("Whether to utilize the color camera image. 是否使用彩色摄像机图像。")]
	public bool computeColorMap = false;
	
	[Tooltip("Whether to utilize the IR camera image. 是否使用红外摄像机图像。")]
	public bool computeInfraredMap = false;
	
	[Tooltip("Whether to display the user map on the screen. 是否在屏幕上显示用户地图。")]
	public bool displayUserMap = false;
	
	[Tooltip("Whether to display the color camera image on the screen. 是否在屏幕上显示彩色摄像机图像。")]
	public bool displayColorMap = false;
	
	[Tooltip("Whether to display skeleton lines over the the user map. 是否在用户地图上显示骨架线。")]
	...
	[Tooltip("Depth and color image width on the screen, as percent of the screen width. The image height is calculated depending on the width. 屏幕上的深度和彩色图像宽度，以屏幕宽度的百分比表示。根据宽度计算图像高度。")]
	public float DisplayMapsWidthPercent = 20f;
	
	[Tooltip("Whether to use the multi-source reader, if one is available (K2-only feature). 是否使用多源读卡器（如果有）（仅限K2功能）。")]
	...
	[Tooltip("Minimum distance to a user, in order to be considered for skeleton data processing.与用户的最小距离，以便考虑骨架数据处理。")]
	public float minUserDistance = 0.5f;
	
	[Tooltip("Maximum distance to a user, in order to be considered for skeleton data processing. Value of 0 means no maximum distance limitation. 与用户的最大距离，以便考虑骨架数据处理。值为0表示没有最大距离限制。")]
	public float maxUserDistance = 0f;
	
	[Tooltip("Maximum left or right distance to a user, in order to be considered for skeleton data processing. Value of 0 means no left/right distance limitation.与用户的最大左或右距离，以便考虑骨架数据处理。值为0表示没有左/右距离限制。")]
	public float maxLeftRightDistance = 0f;
	
	[Tooltip("Maximum number of users, which may be tracked simultaneously. 可同时跟踪的最大用户数。")]
	public int maxTrackedUsers = 6;

	[Tooltip("Whether to display the tracked users within the allowed distance only, or all users (higher fps). 是只显示允许距离内的跟踪用户，还是显示所有用户（更高的fps）。")]
	public bool showTrackedUsersOnly = true;

	public enum UserDetectionOrder : int { Appearance = 0, Distance = 1, LeftToRight = 2 }
	[Tooltip("How to assign users to player indices - by order of appearance, distance or left-to-right. 如何将用户分配到玩家索引-按外观、距离或从左到右的顺序。")]
	public UserDetectionOrder userDetectionOrder = UserDetectionOrder.Appearance;

	[Tooltip("Whether to utilize only the really tracked joints (and ignore the inferred ones) or not. 是否只使用真正跟踪的关节（忽略推断的关节）。")]
	public bool ignoreInferredJoints = false;
	
	[Tooltip("Whether to ignore the Z-coordinates of the joints (i.e. to use them in 2D-scenes) or not. 是否忽略关节的Z坐标（即在2D场景中使用它们）。")]
	public bool ignoreZCoordinates = false;
	
	[Tooltip("Whether to update the AvatarControllers in LateUpdate(), instead of in Update(). Needed for Mecanim animation blending. 是否在LateUpdate（）中而不是在update（）中更新AvatarController。需要Mecanim动画混合。")]
	public bool lateUpdateAvatars = false;
	
	[Tooltip("Whether to skip the remote avatar controllers in multiplayer games. 在多人游戏中是否跳过远程化身控制器")]
...
//	public bool useOwnThread = false; 
	...
	[Tooltip("Set of joint smoothing parameters. 关节平滑参数集。")]
	public Smoothing smoothing = Smoothing.Default;
	
	[Tooltip("Whether to apply the bone orientation constraints.是否应用骨骼方向约束。")]
	public bool useBoneOrientationConstraints = false;
	//public bool useBoneOrientationsFilter = false;

    [Tooltip("Whether to estimate the body joints velocities. 是否估计人体关节速度。")]
	public bool estimateJointVelocities = false; 

	[Tooltip("Set of joint velocities smoothing parameters.关节速度平滑参数集.")]
    public Smoothing velocitySmoothing = Smoothing.Light;

	[Tooltip("Whether to allow detection of body turn arounds or not. 是否允许检测车身转弯。")]
	public bool allowTurnArounds = false;
	
	public enum AllowedRotations : int { None = 0, Default = 1, All = 2 }
	[Tooltip("Allowed wrist and hand rotations: None - no hand rotations are allowed, Default - hand rotations are allowed except for the twists, All - all rotations are allowed.允许的手腕和手旋转：无-不允许手旋转，默认-除了扭曲，允许全部-所有旋转。")]
	public AllowedRotations allowedHandRotations = AllowedRotations.Default;

	[Tooltip("Wait time in seconds, before a lost user gets removed. This is to prevent sporadical user switches. 在删除丢失的用户之前的等待时间，以秒为单位。这是为了防止偶尔的用户切换。")]
	public float waitTimeBeforeRemove = 1f;

	[Tooltip("List of the avatar controllers in the scene. If the list is empty, the available avatar controllers are detected at the scene start up.场景中化身控制器的列表。如果列表为空，则在场景启动时检测到可用的化身控制器。")]
	public List<AvatarController> avatarControllers = new List<AvatarController>();
	
	[Tooltip("Calibration pose required, to turn on the tracking of respective player.需要校准姿势，以打开对各自玩家的跟踪。")]
	public KinectGestures.Gestures playerCalibrationPose;
	
	[Tooltip("List of common gestures, to be detected for each player. 为每个玩家检测的常用手势列表。")]
	public List<KinectGestures.Gestures> playerCommonGestures = new List<KinectGestures.Gestures>();

	[Tooltip("Minimum time between gesture detections (in seconds). 手势检测之间的最短时间（秒）。")]
	public float minTimeBetweenGestures = 0.7f;
	
	[Tooltip("Gesture manager, used to detect programmatic Kinect gestures. 手势管理器，用于检测程序化的Kinect手势。")]
	public KinectGestures gestureManager;
	
	[Tooltip("List of the gesture listeners in the scene. If the list is empty, the available gesture listeners will be detected at the scene start up. 场景中手势侦听器的列表。如果列表为空，则在场景启动时将检测到可用的手势侦听器。")]
	public List<MonoBehaviour> gestureListeners = new List<MonoBehaviour>();

	[Tooltip("GUI-Text to display user detection messages.显示用户检测消息的GUI文本。")]
	public UnityEngine.UI.Text calibrationText;
	
	[Tooltip("GUI-Text to display debug messages for the currently tracked gestures.显示当前跟踪手势的调试消息的GUI文本。")]
