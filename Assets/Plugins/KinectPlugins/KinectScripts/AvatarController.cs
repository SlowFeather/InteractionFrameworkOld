...
{	
	[Tooltip("Index of the player, tracked by this component. 0 means the 1st player, 1 - the 2nd one, 2 - the 3rd one, etc.此组件跟踪的播放机的索引。0表示第一个玩家，1-第二个玩家，2-第三个玩家，等等。")]
	public int playerIndex = 0;
	
	[Tooltip("Whether the avatar is facing the player or not.无论化身是否面向玩家。")]
	public bool mirroredMovement = false;
	
	[Tooltip("Whether the avatar is allowed to move vertically or not.是否允许头像垂直移动。")]
	public bool verticalMovement = false;

	[Tooltip("Whether the avatar's root motion is applied by other component or script.化身的根运动是否由其他组件或脚本应用。")]
	public bool externalRootMotion = false;

    [Tooltip("Whether the head rotation is controlled externally (e.g. by VR-headset)VR是否由耳机外部控制旋转" )]
    public bool externalHeadRotation = false;
	
	[Tooltip("Whether the finger orientations are allowed or not.是否允许手指方向。")]
	public bool fingerOrientations = false;
	
	[Tooltip("Rate at which the avatar will move through the scene.化身在场景中移动的速率。")]
	public float moveRate = 1f;
	
	[Tooltip("Smooth factor used for avatar movements and joint rotations.用于头像移动和关节旋转的平滑因子。")]
	public float smoothFactor = 10f;
	
	[Tooltip("Game object this transform is relative to (optional).此变换相对于的游戏对象（可选）。")]
	public GameObject offsetNode;

	[Tooltip("If enabled, makes the avatar position relative to this camera to be the same as the player's position to the sensor.如果启用，则使化身相对于此相机的位置与玩家对传感器的位置相同。")]
	public Camera posRelativeToCamera;

	[Tooltip("Whether the avatar's position should match the color image (in Pos-rel-to-camera mode only).化身的位置是否应与彩色图像匹配（仅在Pos模式下与相机模式相对）。")]
	public bool posRelOverlayColor = false;
	
	[Tooltip("Whether z-axis movement needs to be inverted (Pos-Relative mode only).是否需要反转z轴移动（仅限Pos相对模式）。")]
	public bool posRelInvertedZ = false;
	
	[Tooltip("Whether the avatar's feet must stick to the ground.阿凡达的脚是否必须贴在地上。")]
	public bool groundedFeet = false;

	[Tooltip("Whether to apply the humanoid muscle limits or not.是否应用人形肌肉限制。")]
	...
	[Tooltip("Vertical offset of the avatar with respect to the position of user's spine-base.化身相对于用户脊椎底部的位置的垂直偏移。")]
	[Range(-0.5f, 0.5f)]
	public float verticalOffset = 0f;

	[Tooltip("Forward (Z) offset of the avatar with respect to the position of user's spine-base.相对于用户脊椎底部的位置，化身的向前（Z）偏移。")]
	[Range(-0.5f, 0.5f)]
